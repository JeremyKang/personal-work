...
xySplit(c(1,2,3,4,645,234,63,1,6,8,9))
...
includePR = function(df){
  pr = df[,'hp']/df[,'wt']
  df <- data.frame('pr' = c(pr), df)
  return(df)
}

pr = mtcars[,'hp']/mtcars[,'wt']
pr
...
df = includePR(mtcars)
df
stopifnot(df["Maserati Bora","pr"] == df["Maserati Bora","hp"] / df["Maserati Bora", "wt"])
...
max_qsec=16
min_mpg=15
mqs = df[df['qsec'] <= max_qsec,]
mpg = df[df['mpg'] >= min_mpg,]
mqs
mpg

mqs
subset(mtcars, qsec <= max_qsec & mpg >= min_mpg)

print( subset(mtcars, hp >= 160 & mpg >= 18) )


fastButEfficient(mtcars, 16, 15)
...
median_d_c = aggregate(diamonds["price"], by = diamonds["cut"], FUN=median)
median_d_c
highest = max(median_d_c['price'])  
row <- (median_d_c[['price']] == highest)

median_d_c
as.character(median_d_c[1,'cut'])
median_d_c[['price']] == highest

which(median_d_c['price'] == max(median_d_c['price']))
...
cutWithGreatestMedianPrice = function(df){
  median_d_c = aggregate(df["price"], by = df["cut"], FUN=median)
  ind = which(median_d_c['price'] == max(median_d_c['price']))
  
  return(as.character(median_d_c[ind,'cut']))
}

print(head(cutWithGreatestMedianPrice(diamonds)))
head(diamonds)
...
comboWithGreatestMedianPrice = function(df){
  median_cut = aggregate(df["price"], by = df["cut"], FUN=median)
  ind = which(median_cut['price'] == max(median_cut['price']))
  
  median_color = aggregate(df["price"], by = df["color"], FUN=median)
  ind = which(median_color['price'] == max(median_color['price']))
  
  median_clarity = aggregate(df["price"], by = df["clarity"], FUN=median)
  ind = which(median_clarity['price'] == max(median_clarity['price']))
  
  print(median_cut)
  print(median_color)
  print(median_clarity)
}

cutWithGreatestMedianPrice = function(df){
  x = aggregate(df["price"], by=df["cut"], median)
  x = x[which.max(x$price),"cut"]
  return(levels(x)[x])
}
...
cutWithGreatestMedianPrice = function(df){
  ...
  median_price = median_price[with(median_price, order(price,decreasing = TRUE)), ]
  return (head(median_price$cut,1))
}

comboWithGreatestMedianPrice = function(df) {
  median_prices = aggregate(df["price"], by = c(df["cut"],
                                                df["color"],
                                                df["clarity"]),
                         FUN = median)
  high = which.max(median_prices[, "price"])
  res = median_prices[high, ]
  return(res)
}
