...
df5Testp1C2 <- df5Testp1[,c("Gross", "Budget", "RuntimeCut", Genre_edited_names)]
df5Testp1C2 <- gather(df5Testp1C2, Genre_edited_names, Genre_presence, -c(Gross, Budget, RuntimeCut))

#Remove rows in which Genre is not presesnt
setup_df5Testp1C2 <- df5Testp1C2[!(apply(df5Testp1C2, 1, function(x) any(x == 0))),]

#Remove Genre_presence
setup_df5Testp1C2$Genre_presence = NULL
#setup_df5Testp1C2 <- setup_df5Testp1C2[!is.na()]

# ggplot(setup_df5Testp1C2, aes(x=Budget, y=Gross)) +
#   geom_point() +
#   facet_grid(RuntimeCut~Genre_edited_names) +
#   labs(title="")

IQR(setup_df5Testp1C2$Gross[setup_df5Testp1C2$Genre_edited_names == 'Genre_Adventure'], na.rm=TRUE)
IQR(setup_df5Testp1C2$Gross[setup_df5Testp1C2$Genre_edited_names == 'Genre_Animation'], na.rm=TRUE)

#Gross revenue by budget
ggplot(setup_df5Testp1C2, aes(reorder(Budget, Gross), Gross)) +
  geom_point() +
  labs(title="Gross Revenue by Budget",
       x="Budget",
       y="Gross Revenue")

#Gross revenue by budget (both transformed)
ggplot(setup_df5Testp1C2, aes(reorder(log(Budget), log(Gross)), log(Gross))) +
  geom_point() +
  labs(title="log(Gross Revenue) by log(Budget)",
       x="log(Budget)",
       y="log(Gross Revenue)")

#Gross revenue by genre
ggplot(setup_df5Testp1C2, aes(reorder(Genre_edited_names, Gross), Gross, fill=Genre_edited_names)) +
  geom_boxplot(show.legend=FALSE) +
  coord_flip() +
  labs(title="Gross Revenue Distribution by Genres",
       x="Genre",
       y="Gross Revenue")

#Log of gross by runtime
ggplot(df, aes(reorder(Runtime, log(Gross)), log(Gross))) +
  geom_point() +
  labs(title="Gross Revenue by Runtime",
       x="Runtime",
       y="Gross Revenue")

#Gross revenue by runtime
ggplot(setup_df5Testp1C2, aes(reorder(RuntimeCut, Gross), Gross, fill=RuntimeCut)) +
  geom_boxplot(show.legend=FALSE) +
  coord_flip() +
  labs(title="Gross Revenue Distribution by Runtime (Cut into 5 buckets)",
       x="Runtime (Cut)",
       y="Gross Revenue")
...
**A**: Plotting the raw numbers for budget and gross appear toshow an exponential pattern. However, it is difficult to tell if the relationship is linear or not. Plotting the log log plot of Budget and Gross Revenue shows a clearer trend in the data. We see a lot of movies that are in the first quartile range of the budget, and although many appear to be successfully increasing the gross revenue, there seems to be a certain optimal budget in which movies are given. 

Another interesting pattern emerges in the upper quartiles where even certain movies follow an O(log n) pattern all the way to the highest budget. Of course, there are many movies with deviations to this line, but the pattern is very distinct and, again, suggests that movie companies are cautiously allocating budgets to certain movies and it is paying off in gross revenue. The highest IQR is in animation at $332,962,714 suggesting high variability in gross revenue returned with animated films. The second IQR highest is with Adventure at 
$299,154,450, however there are more outliers with adventure movies. 

Adventure, SciFi, and Action genres have the largest outliers (and are, actually the same movie [Avatar]). The next highest is a movie is Titanic which was a Romance and Drama. Other than these two movies, gross revenue outliers exist in, almost all of the Genres (excluding adult, news, and film noir). A somewhat interesting pattern in the plot of genres and gross revenue is that there appears to be a minimum gross revenue across all genres, including adult, news, and film noir. 

Lastly, there doesn't seem to be any apparent relationship between runtime and gross revenue. Although, it's interesting to note that the majority of movies have similar runtimes and, in turn, similar gross revenues. When runtime is bucketed into five ranges, you can see that runtimes of 729 to 110 minutes have the highest gross revenue movies. 
```{r}
# TODO: Investigate if Gross Revenue is related to Release Month
df$MonthReleased <- as.numeric(month(df$Released))

ggplot(df, aes(MonthReleased, Gross)) +
  geom_point() +
  labs(title="Gross Revenue by Month Movie was Released",
        x="Month",
        y="Gross Revenue")

```
**Explanation**: Although December appears to have some outliers, it appears movies released between April and July have the highest gross revenue.
...
convert_Awards_wins = function(dfAwardsCol) {
  Wins = vector(length=length(df$Awards))
...
df$Awards_wins <- convert_Awards_wins(df$Awards)
#df$Awards_check <- dfmergeTest$Awards


#Convert Award nominations function
convert_Awards_nom = function(dfAwardsCol) {
  Nom = vector(length=length(df$Awards))
  for (i in 1:length(dfAwardsCol)) {
    if (dfAwardsCol[i] == 'N/A') {
        Nom[[i]] = NA
...
df$Awards_nominations <- convert_Awards_nom(df$Awards)

length(which(df$Awards_wins >= 1))
length(which(df$Awards_nominations >= 1))
df["Awards"] <- NULL
...
**A**: I created a custom function that takes in a df_col (i.e., df$Awards) and returns a new vector of the same length with a numeric value for each award and (in a separate, but using the same logic) function, nomination. The function begins with creating an empty vector. It iterates through each row in the column, first checking if it is the string "N/A", and converting it into NA for R to better process. It the begins to search for if the 'Won [x]' or 'Nominated for [x]' string is present. If it is, then capture the index in which it found the substring and use that index to retrieve the numeric value. Note the start and end of the string to capture the numeric value was hard coded and not optimized to be flexible with patterns not seen in the Awards column. The function then moves to check if there are any additional wins or nominations and retrieves that number. The function was also required to check strings that were without any other text (e.g., '6 wins'). When that number was captured, all if the wins were then summed and then added to the empty vector. 

4127 movies had valid/non-zero wins. 4204 movies had valid/non-zero nominations.

```{r}
# TODO: Plot Gross revenue against wins and nominations
#Gross by Wins
ggplot(df, aes(Awards_wins, Gross)) +
  geom_point() +
  stat_smooth() +
  labs(title="Gross Revenue by Award Wins",
        x="Award Wins",
        y="Gross Revenue")

#Gross by Nominations
ggplot(df, aes(Awards_nominations, Gross)) +
  geom_point() +
  stat_smooth() +
  labs(title="Gross Revenue by Award Nominations",
        x="Award Nominations",
        y="Gross Revenue")

#Log on Gross by Wins
ggplot(df, aes(Awards_wins, log(Gross))) +
  geom_point() +
  stat_smooth() +
  labs(title="Gross Revenue by Award Wins",
        x="Award Wins",
...
#Log on Gross by Nominations
ggplot(df, aes(Awards_nominations, log(Gross))) +
  geom_point() +
  stat_smooth() +
  labs(title="Gross Revenue by Award Nominations",
        x="Award Nominations",
...
**A**: The log of gross revenue and award wins show a linear relationship. The log of gross revenue and award nominations show a slight exponential increase, but flat lines at around 15 nominations. 
...
condensed_ratings <- c("imdbRating", "tomatoMeter", "tomatoRating", "tomatoUserMeter", "tomatoUserRating", "tomatoFresh", "tomatoRotten")
...
**A**: Although, the Tomato Meter is Rotten Tomatoes main critic ratings, Tomato Rating seems to be on the same scale as IMDb ratings (1-10) and also seems to be a high correlation between these two at 0.81 - even higher than the correlation between the Tomato Meter and IMDb ratings at 0.76. This is despite the fact that Tomato Ratings and Tomato Meter has a correlation of 0.96. Tomato Rotten is relatively a decent predictor in all other Rotten Tomatoes metrics (not surprising), but also shows a -0.34 correation with IMDb ratings. 

User ratings and user meter from Rotten Tomatoes is fairly predictive of IMDb ratings, at 0.84 and 0.81, respectively. 
...
**A**: Tomatoe Ratings has the highest correlation with both award wins and award nominations at 0.5 and 0.45, respectively. Somewhat surprisngly, Tomato User Meter and User ratings have the lowest prediction rate in award wins and nominations. This suggest that professionals who decides nominations and wins are quite different than the general Rotten Tomatoes user. The most intriguing aspect of this plot is that IMDb ratings also had low predictive power for award wins and nominations at 0.44 and 0.34, respectively. However, if you read how IMDb ratings are calculated, it is again, the general user who votes, not the professional critics of Rotten Tomatoes. 
